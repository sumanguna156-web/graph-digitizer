                ...
                for gi, g in enumerate(extracted):
                    ...
                    try:
                        ...
                        out = {"id": gid, "page": pi,
                               ...
                               "curves": curves}
                        yield sse("graph", out)
                    except Exception as e:  # noqa: BLE001 — never let one region kill the stream
                        yield sse("graph_error", {"id": gid, "page": pi, "title": gid,
                                                  "message": f"skipped: {type(e).__name__}"})
                        continue
...
async def rescore(payload: dict, code: str = ""):
    """Recompute per-curve match% against the original image after edits.
    payload: {uid, page, box:[x,y,w,h], curves:[{color,x,y}]}"""
    _check_code(code)
    uid = payload.get("uid")
    item = UPLOADS.get(uid)
    if not item or "pages" not in item:
        raise HTTPException(404, "Upload expired. Re-run extraction to enable accuracy check.")
    pi = int(payload.get("page", 1))
    img = item["pages"].get(pi)
    if img is None:
        raise HTTPException(400, "No such page.")
    ...
    if not box or len(box) != 4:
        raise HTTPException(400, "box required.")
    x, y, w, h = [int(v) for v in box]
    crop = img[y:y+h, x:x+w]
    xa = payload.get("x_axis", {}); ya = payload.get("y_axis", {})
    # rebuild calibration mapping from ticks/min/max/scale sent by the client
    def mk(ax, size, flip=False):
        # flip=True for the y axis: image pixel 0 is the TOP of the plot = max value
        ...
        if scale == "log" and lo > 0 and hi > 0:
            ...
            if flip:
                return {"scale": "log", "min": lo, "max": hi,
                        "px2val": (lambda p, L0=l0, L1=l1, S=size: 10**(L1 - p/S*(L1-L0)))}
            return {"scale": "log", "min": lo, "max": hi,
                    "px2val": (lambda p, L0=l0, L1=l1, S=size: 10**(L0 + p/S*(L1-L0)))}
        if flip:
            return {"scale": "linear", "min": lo, "max": hi,
                    "px2val": (lambda p, LO=lo, HI=hi, S=size: HI - p/S*(HI-LO))}
        return {"scale": "linear", "min": lo, "max": hi,
                "px2val": (lambda p, LO=lo, HI=hi, S=size: LO + p/S*(HI-LO))}
    cx = mk(xa, w); cy = mk(ya, h, flip=True)
    out = []
    for c in payload.get("curves", []):
        out.append(D.score_curve_vs_image(c, crop, cx, cy, w, h))
    return {"match": out}
