...
def find_plot_boxes(img):
    ...
    # long horizontal & vertical lines only (frame)
    h_k = cv2.getStructuringElement(cv2.MORPH_RECT, (60, 1))
    v_k = cv2.getStructuringElement(cv2.MORPH_RECT, (1, 60))
    ...
    for c in cnts:
        ...
        if w > 400 and h > 400 and w < 0.9*W and h < 0.9*H:
    ...
    for b in boxes:
        ...
        for k in keep:
            kx,ky,kw,kh = k
            if abs(x-kx)<30 and abs(y-ky)<30 and abs(w-kw)<60 and abs(h-kh)<60:
                dup = True; break
            # contained deep inside an existing kept box -> inner artifact
            if x>kx+40 and y>ky+40 and x+w<kx+kw-40 and y+h<ky+kh-40:
                dup = True; break
        if not dup: keep.append(b)
    return sorted(keep, key=lambda b: (round(b[1]/300), b[0]))  # reading order
...
def _dedupe(curves, xr, yr, tol=0.02):
    ...
    for d in sorted(curves, key=lambda d: (span(d), len(d)), reverse=True):
        dup = False
        xs = np.array([a for a, b in d]); ys = np.array([b for a, b in d])
        for k in keep:
            kx = np.array([a for a, b in k]); ky = np.array([b for a, b in k])
            lo, hi = max(xs.min(), kx.min()), min(xs.max(), kx.max())
            if hi - lo < 0.3*xr: continue
            q = np.linspace(lo, hi, 40)
            o1, o2 = np.argsort(xs), np.argsort(kx)
            diff = np.abs(np.interp(q, xs[o1], ys[o1]) - np.interp(q, kx[o2], ky[o2]))
            if np.median(diff) < tol*yr: dup = True; break
