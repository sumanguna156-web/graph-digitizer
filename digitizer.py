...
def digitize_page_img(img, boxes=None):
    ...
    for bi, box in enumerate(boxes):
        ...
        for hexc, d in raw:
            ...
            c['match'] = score_curve_vs_image(c, crop, cx, cy, w, h)
            curves.append(c)
...
def _curve_inverse(cal, size, n=250):
    import numpy as _np
    px = _np.linspace(0, size, n)
    vals = _np.array([cal['px2val'](p) for p in px])
    o = _np.argsort(vals)
    return vals[o], px[o]

def score_curve_vs_image(curve, crop, cx, cy, w, h, band=7, tol=100):
    """Return 0-100: fraction of the traced polyline landing on the curve's
    real colored pixels in the source image. Measures trace accuracy honestly."""
    import numpy as _np
    try:
        vx, pxx = _curve_inverse(cx, w); vy, pxy = _curve_inverse(cy, h)
        col = curve.get('color', '#000000')
        tgt = _np.array([int(col[5:7],16), int(col[3:5],16), int(col[1:3],16)])
        diff = _np.abs(crop.astype(int) - tgt).sum(axis=2)
        mask_color = diff < tol
        # Black-ink datasheets are redrawn in display colors that don't exist in
        # the source, so also verify against dark ink and keep the better score.
        import cv2 as _cv2
        mask_dark = _cv2.erode((crop.mean(axis=2) < 90).astype(_np.uint8),
                               _np.ones((5,5), _np.uint8)).astype(bool)
        xs, ys = curve['x'], curve['y']
        def run(mask):
            hits = total = 0
            for i in range(len(xs)-1):
                for t in _np.linspace(0, 1, 15):
                    vxx = xs[i]+(xs[i+1]-xs[i])*t; vyy = ys[i]+(ys[i+1]-ys[i])*t
                    px = int(_np.interp(vxx, vx, pxx)); py = int(_np.interp(vyy, vy, pxy))
                    if 0 <= px < w and 0 <= py < h:
                        total += 1
                        y0,y1 = max(0,py-band),min(h,py+band+1)
                        x0,x1 = max(0,px-band),min(w,px+band+1)
                        if mask[y0:y1, x0:x1].any(): hits += 1
            return round(hits/total*100) if total else None
        s1 = run(mask_color)
        s2 = run(mask_dark) if mask_dark.sum() > 500 else None
